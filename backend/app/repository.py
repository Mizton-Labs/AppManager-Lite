...
def render_connect_scripts(
    ...
    *,
    ...
    One script per usable server (not failed, has an IP), matching the config's
    ``Host`` alias and filter. Returns ``(filename, script_text)`` pairs.

    - Standard mode (builtin/generic configs): the script defers entirely to the
      bundled ``config`` via ``ssh -F ./config <alias>``, so it inherits User,
      IdentityFile, ProxyJump/jumpserver, and keepalives.
    - Self-contained mode (mapping templates, whose text may not be a standard
      ssh config): the script passes the key and any jump host explicitly so it
      works regardless of the bundled ``config`` content.
    ...
    for server in user_servers:
        ...
        if self_contained:
            ...
            parts = [
                "ssh",
                "-i",
                # key_name is a strict slug ([a-z0-9_-]); the surrounding double
                # quotes keep $(dirname "$0") as a live path substitution.
                f'"$(dirname "$0")/{key_name}"',
                "-o",
                "IdentitiesOnly=yes",
            ]
            if jump_enabled:
                proxy = f"{jump_user}@{jump_host}:{jump_port}"
                parts += ["-o", "ProxyJump=" + shlex.quote(proxy)]
